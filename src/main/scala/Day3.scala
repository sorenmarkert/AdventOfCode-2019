...
object Day3 {
    ...
    def findIntersections(wire1: List[Leg], wire2: List[Leg]) = {

        def findIntersection(leg1: Leg, leg2: Leg) = {
            ...
            (leg1, leg2) match {
                case (v: VerticalLeg, h: HorizontalLeg) => crossPoint(v, h)
                case (h: HorizontalLeg, v: VerticalLeg) => crossPoint(v, h)
                case _ => None
            }
